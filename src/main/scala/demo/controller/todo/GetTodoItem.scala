        ...
        path("all") {
          headerValueByName("Auth-Token") { token =>
            ...
            if (userID == -1) {
              ...
              transaction {
                val itemList = DataBase.todoItems.where(item => item.userID === userID).toList.map(x => Item(x.id, x.content, x.done, x.startAt.toString))
                complete(StatusCodes.Accepted, HttpEntity(ContentTypes.`application/json`,
                  TodoItemListResponse(
                    body = Map("data" -> itemList)).toJson.toString
                ))
              }
            }
          }
